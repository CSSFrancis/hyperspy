...
class SeqReader(object):
    ...
    def parse_metadata_file(self):
        ...
        try:
            with open(self.file + ".metadata", 'rb') as meta:
                ...
                try:
                    scale = ''.join([chr(a) for a in m[6] if a!=0]).split()
                    print(scale)
                    self.metadata_dict["PixelUnit"] = scale[1]
                except(IndexError):
                    _logger.info('No Scale found')
                    self.metadata_dict["PixelUnit"]=None
    ...
    def create_axes(self):
        ...
        axes.append({'name': 'y', 'offset': 0, 'scale': 1, 'size': self.image_dict["ImageHeight"],
                     'navigate': False, 'index_in_array': 1, 'units':"undefined"})
        axes.append({'name': 'x', 'offset': 0, 'scale': 1, 'size': self.image_dict["ImageWidth"],
                     'navigate': False, 'index_in_array': 2, 'units': "undefined"})
        ...
        if self.metadata_dict is not {} and self.metadata_dict["PixelSize"] != 0:
            ...
            axes[1]['units'] = self.metadata_dict["PixelUnit"]
            axes[2]['units'] = self.metadata_dict["PixelUnit"]
        ...
        if self.metadata_dict is not {} and self.metadata_dict["DiffPixelSize"] != 0:
            ...
            axes[1]['name'] = self.metadata_dict["kx"]
            axes[2]['name'] = self.metadata_dict["ky"]
    ...
    def _apply_correction(self, start, dtype_list):
        self.open_file.seek(start)
        if self.dark_ref is not None and self.gain_ref is not None:
            d = np.fromfile(self.open_file, dtype_list, count=1)
    ...
    def get_image_data(self, lazy=False):
        ...
        self.open_file = open(self.file, mode='rb')
        ...
        starts = [8192 + i * self.image_dict["ImgBytes"]for i in range(self.image_dict["NumFrames"])]
        if lazy:
            from dask import delayed
            from dask.array import from_delayed, stack
            values = [delayed(self._apply_correction)(start=start, dtype_list=dtype_list) for start in starts]
            arrays = [from_delayed(v,
                                   shape=(self.image_dict["ImageWidth"], self.image_dict["ImageHeight"]),
                                   dtype=self.image_dict["ImageBitDepth"])
                      for v in values]
            data = stack(arrays)
        else:
            data = np.array([self._apply_correction(start=start, dtype_list=dtype_list) for start in
                     starts])
...
def file_reader(filename, lazy=False):
    ...
    data = seq.get_image_data(lazy=lazy)
    dictionary = {
        'data': data,
        'metadata': metadata,
        'axes': axes,
